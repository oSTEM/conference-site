...
<div className="text-l flex justify-center p-2">
  <p className='text-lg text-yellow-800 dark:text-yellow-100 bg-yellow-500/10 px-3 rounded-lg'><i>RFPs have closed.</i></p>
</div>
...
<Tab.List className='z-10 mt-4 py-1 sm:py-2 sticky top-[87px] sm:top-[63px] flex space-x-1 bg-primary border-b border-text-color'>{
    ['Collegiate', 'Industry', 'Technical Spotlight'].map(e => (<Tab key={e} className={({selected}) => (`${selected ? `bg-blue-300/25 border-nav-blue/90` : `hover:bg-blue-300/15`} text-nav-blue border border-transparent transition text-sm sm:text-base rounded px-1 sm:px-2 py-0.5`)}>{e}</Tab>))
}</Tab.List>
